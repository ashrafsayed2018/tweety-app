<div class="flex">
    ...
    <form action="/tweets/{{ $tweet->id }}/likes" method="POST">
        @csrf
        <button type="submit" class="flex items-center w-6 mr-4 cursor-pointer focus:outline-none">
            <svg viewBox="0 0 20 20" class=" {{ $tweet->isLikedBy(auth()->user()) ? 'text-blue-500' : 'text-gray-500' }} mr-1" style="transform: scale(-1,1);">
    ...
    <form action="/tweets/{{ $tweet->id }}/likes" method="POST">
        @csrf
        @method('DELETE')
        <button type="submit" class="flex items-center w-6 cursor-pointer  focus:outline-none">
            <svg viewBox="0 0 20 20"  class=" {{ $tweet->isDislikedBy(auth()->user()) ? 'text-blue-500' : 'text-gray-500' }} mr-1">
                <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                    <g class="fill-current">
